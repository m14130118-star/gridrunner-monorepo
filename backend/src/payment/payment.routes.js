...
router.post('/create-session', (req, res) => {
  ...
  const session = db.insert('payment_sessions', {
...
router.post('/confirm', (req, res) => {
  ...
  const session = db.findById('payment_sessions', session_id);
  if (!session) return res.status(404).json({ success: false, message: 'Session not found' });

  const plan = PLANS[session.plan_id];
  const account = db.findById('accounts', session.user_id);
  account.vip = true;
  account.vip_until = new Date(Date.now() + plan.duration_days * 86400000).toISOString();
  db.update('accounts', session.user_id, account);
  db.update('payment_sessions', session_id, { status: 'completed' });

  // Achievement unlock
  db.insert('achievements', {
    user_id: session.user_id, name: 'VIP Статус',
...
router.get('/history', (req, res) => {
  const sessions = db.query('payment_sessions', s => s.user_id === req.user.id);
