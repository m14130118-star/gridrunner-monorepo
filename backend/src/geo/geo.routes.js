...
router.post('/poi/suggest', authenticate, (req, res) => {
  ...
  // Reward suggester
  const account = db.findById('accounts', req.user.id);
  if (account) {
    account.xp = (account.xp || 0) + 50;
    db.update('accounts', req.user.id, account);
  }
...
router.get('/poi/pending', authenticate, requireRole('admin'), (req, res) => {
  const pending = db.query('pois', p => p.approved === false && p.is_active !== false);
...
router.get('/osm/fetch', async (req, res) => {
  ...
  try {
    ...
    const pois = (data.elements || []).map(el => {
      ...
      const existing = db.findOne('pois', p => p.osm_id === el.id);
...
router.post('/traps', authenticate, requireRole('player'), (req, res) => {
  const { latitude, longitude, radius_km, damage_hp, name, clan_id } = req.body;
  const trap = db.insert('traps', {
...
router.get('/traps', authenticate, (req, res) => {
  const traps = db.query('traps', t => t.is_active);
  res.json({ success: true, traps });
});

router.delete('/traps/:id', authenticate, (req, res) => {
  db.remove('traps', parseInt(req.params.id));
  res.json({ success: true });
});

// Districts (PostGIS polygon data) - simplified with center point + radius
router.get('/districts', (req, res) => {
  const districts = db.load('districts');
  res.json({ success: true, districts });
});

router.get('/districts/:id/capture', authenticate, (req, res) => {
  const history = db.query('capture_history', h => h.district_id === parseInt(req.params.id));
...
function mapOsmCategory(tags) {
  ...
  if (['cafe', 'restaurant', 'fast_food', 'bar', 'pub'].includes(amenity)) return 'food';
  if (['park', 'garden', 'playground'].includes(amenity)) return 'park';
  if (['cinema', 'theatre', 'museum', 'art_gallery'].includes(amenity)) return 'culture';
  if (['sports_centre', 'swimming_pool', 'fitness_centre'].includes(amenity)) return 'sport';
  if (['bicycle_parking', 'bicycle_rental'].includes(amenity)) return 'bike';
  return 'other';
}
