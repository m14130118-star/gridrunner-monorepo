import type { RoutePlanRequest, RoutePlanResponse, Place, BucketKey, Vehicle, PlacePref } from './types';
import { SPEEDS, MAX_RADIUS_KM, VISIT_TIMES, DEFAULT_VISIT_TIME, PREF_TAGS, VEHICLE_TAGS, haversineKm, estimateRouteTime, CATEGORY_LABELS } from './constants';

export async function planRouteClient(req: RoutePlanRequest): Promise<RoutePlanResponse> {
  const { lat, lng, vehicle, duration, prefs } = req;
  if (!lat || !lng || !vehicle || !duration) throw new Error('Missing fields');
  ...
  return { city, vehicle: vehicle as Vehicle, duration, places: scored.slice(0, 30), buckets };
}
...
async function searchPlaces(city: string, lat: number, lng: number, vehicle: Vehicle, prefs?: PlacePref[]): Promise<Place[]> {
  ...
  for (const term of queryTerms) {
    try {
      const q = term.includes('=') ? term.split('=').pop() : term;
...
function fallbackPlaces(city: string, lat: number, lng: number, vehicle: Vehicle, prefs?: PlacePref[]): Place[] {
  ...
  const places: Place[] = [];
  const names = prefs && prefs.length > 0 ? prefs : ['park', 'cafe', 'attraction', 'viewpoint', 'museum', 'park'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    places.push({
      id: `fallback_${i}`, name: `${label} — ${city}`,
      lat: lat + dLat, lng: lng + dLng,
      osmType: 'node', category: pref, tags: [],
      visitTime: 0, routeTime: 0, totalTime: 0, distance: 0,
    });
  }
  return places;
