    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center px-6">
        <div className="w-full max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">{lang === 'ru' ? 'Аккаунт создан!' : 'Account created!'}</h2>
          <p className="text-[#94a3b8] mb-8">{lang === 'ru' ? 'Теперь скачай приложение и войди в него' : 'Download the app and sign in to start playing'}</p>
          ...
          <p className="mt-4 text-sm text-[#64748b]"><Link href="/auth/login" className="text-[#7c3aed] hover:text-[#6d28d9]">{lang === 'ru' ? 'Войти' : 'Sign in'}</Link></p>
        </div>
      </div>
    );
  }
